...
plt.errorbar(y= delta_l/L_0, x=T, yerr=u_L_by_L_0, xerr=u_T ,fmt='o', capsize=5, markersize = 0, label='Dane z błędami')

plt.xlim(0)  
plt.ylim(0) 

a, b = np.polyfit(T, delta_l/L_0, 1)
x_reg = np.linspace(0, 150, 1000) 
print(f"y = {a}*x + {b}")


plt.plot(x_reg, a*x_reg + b, x_reg)
plt.show()